...
def calculate_courirs_daily_income(sender, instance, created, **kwargs):
    if created:
        try:
            daily_income = DailyIncome.objects.get(user = instance.user, date = instance.courier_time.date())
        except DailyIncome.DoesNotExist:
            daily_income = None
        if not daily_income:
            DailyIncome.objects.create(user = instance.user, amount = instance.amount, date = instance.courier_time.date())
            return
        new_amount = instance.amount + daily_income.amount
        DailyIncome.objects.update(amount = new_amount)
...
def calculate_courirs_weekly_income(sender, instance, created, **kwargs):
    if created:
        ...
        WeeklyIncome.objects.update(amount = new_amount)
